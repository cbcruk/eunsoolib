# @cbcruk/use-like-store

찜 목록 상태 관리 유틸리티. **framework-agnostic 싱글턴 core + `useSyncExternalStore` React 레이어** 패턴을 따른다.

- **core**: `@cbcruk/sync-store`의 `createStore`로 만든 싱글턴(`likeStore`). React를 모르므로 단독 테스트/재사용 가능.
...
```bash
pnpm add @cbcruk/use-like-store react
```

`@cbcruk/sync-store`와 `zod`(아이템 스키마 검증)에 의존하며, React 훅을 쓰려면 `react`(peer dependency)가 필요합니다.
